...
function sendCommentAjax(url, data, flag) {
    $.ajax({
        type:'post',
        url:url,
        data:data,
        success:function(commentDTO) {
            ...
            if(commentDTO['board']['structure']==1) {
               ...
               e1 += '<a href="../../../../' + comment['username'] + '/" class="makeUsernameBold">' + comment['username'] + '</a> ';
               if(flag=='1') {
                    e1 += comment['content'];
                    e1 += '<br>';
               ...
               e1 += '<a boardid="' + comment['boardid'] + '" class="likes">&nbsp;좋아요 ' + 0 + '개&nbsp;</a>';
               ...
               e1 += '</div>';
               console.log(e1);
               if(flag=='1') $('#mainContent').after(e1);
               if(flag=='2') $('#content2').prepend(e1);
            } else {
                ...
                e2 += '<a boardid="' + recomment['boardid'] + '" class="likes">&nbsp;좋아요 ' +  0 + '개&nbsp;</a>';
    ...
    $('#content2').on('click', '.heartButton', function(event) {
        ...
        $.ajax({
            ...
            success:function(data) {
                ...
                $('.likes[boardid="'+boardid+'"]').html(text);
            },
            error:function() {
               alert("오류! 관리자에게 문의!");
            }
...
//        }
//    });
    $('#content5').on('input', '#writeComment', function(event) {
        const content = trimData($('#writeComment').val());
        if(content && content.length>0) {
            $('#submitComment').css('display', 'inline');
        } else {
...
//        }
//        if(content) {
//            if(content.length>0) $('#submitComment').css('display', 'inline');
//            else $('#submitComment').css('display', 'none');
//        } else $('#submitComment').css('display', 'none');
    });
    $('#content5').on('keydown', '#writeComment', function(event) {
        if(event.which==13) {
            event.preventDefault();
            if($('#submitComment').css('display')=='block') {
                    $('#submitComment').click();
            } else {
                $('#submitComment').val('');
            }
    ...
    $('#content5').on('click', '#submitComment', function() {
        ...
        if(structure.indexOf('structure2')==-1) {
            ...
            sendCommentAjax('../../../../board/commentInsert', data, flag);
        } else {
